...
def enhancedFeatureExtractorDigit(datum):
    ...
    # helper method to find orthogonally adjacent pixels
    def getAdj(coordinate):
        x = coordinate[0]
        y = coordinate[1]
    ...
    for x in range(DIGIT_DATUM_WIDTH):
        for y in range(DIGIT_DATUM_HEIGHT):
            ...
            brightness = datum.getPixel(x, y) # datum[(x, y)] -> brightness value of 0, 1, 2


            if (x, y) in checkedPixels: # skip this loop if we've already looked at this pixel
                continue

            if brightness != 0: # mark this pixel as checked, then continue if pixel is marked
            ...
            while not region.isEmpty():
                p = region.pop()

                # looping through adj pixels
                for adj in getAdj(p):
                    # add adj to our region if it's unmarked and that has not been checked
                    if adj not in checkedPixels:
                        checkedPixels.add(adj)

                        if datum.getPixel(adj[0], adj[1]) == 0:
                            region.push(adj)

    features["1region"] = 1 if numRegions == 1 else 0
    features["2region"] = 1 if numRegions == 2 else 0
    features["3region"] = 1 if numRegions > 2 else 0
...
def enhancedPacmanFeatures(state, action):
    ...
    # keep track of each ghost as well as closest ghost
    closestGhost = float("inf")
    for i in range(len(ghosts)):
        ghost = ghosts[i]
        dist = util.manhattanDistance(pac, ghost)

        features[("ghost", i)] = 5 / (0.01 + dist)

        if dist < closestGhost:
            closestGhost = 5 / (0.01 + dist)

    features["closestGhost"] = closestGhost

    # keep track of all food as well as closest food
    closestFood = float("inf")
    for i in range(len(food)):
        f = food[i]
        dist = util.manhattanDistance(pac, f)

        features[("food", i)] = 5 / (0.01 + dist)

        if dist < closestFood:
            closestFood = 5 / (0.01 + dist)

    features["closestFood"] = closestFood

    # keep track of number of capsules
    features["numCapsules"] = len(capsules)

    features["win"] = 1 if successor.isWin() else 0
    features["lose"] = 1 if successor.isLose() else 0
